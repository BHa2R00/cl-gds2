...
;; operatiors
(defun sub-xy (st n0 &optional n1)
  (if (and
        (xy-p st)
        (integerp n0)
        (if n1 (integerp n1) t))
    (let*((st1 (copy-structure st))
          (l1 (xy-ivec st1)))
      (if n1
        (setf (xy-ivec st1) (subseq l1 (ash n0 1) (ash n1 1)))
        (setf (xy-ivec st1) (subseq l1 (ash n0 1))))
      st1)
    st))
(defun append-xy (&rest sts) (make-xy :ivec (apply 'append (mapcar #'xy-ivec sts))))
(defun chop-xy (st n)
  (if (xy-p st)
    (let*((st1 (copy-structure st))
          (l1 (xy-ivec st1))
      ...
      st1)
    st))
(defun move-xy (st to &optional range0 range1)
  (if (and
        (xy-p st)
        (xy-p to))
    (let*((st1 (copy-structure st))
            ...
            (map 'list 
                 (lambda (li) 
                   ...
                   (if (and 
                         ...
                         (<= range0 range1))
                     (if (and 
                           (>= k (+ (ash range0 1) (rem k 2)))
                           (< k (+ (ash range1 1) (rem k 2))))
                       (+ li (nth (rem k 2) (xy-ivec to)))
                       li)
                     (+ li (nth (rem k 2) (xy-ivec to))))))
                 l))
      st1)
    st))
(defun scale-xy (st from a &optional range0 range1)
  (if (and
        (xy-p st)
        (xy-p from)
        (numberp a))
             ...
             (lambda (li)
               ...
               (if (and
                 ...
                 (if (and
                       (>= k (+ (ash range0 1) (rem k 2)))
                       (< k (+ (ash range1 1) (rem k 2))))
                   (+ (nth (rem k 2) (xy-ivec from)) (* a (- li (nth (rem k 2) (xy-ivec from)))))
                   li)
                 (+ (nth (rem k 2) (xy-ivec from)) (* a (- li (nth (rem k 2) (xy-ivec from))))))))
             (xy-ivec st1)))
      st1)
    st))
(defun rotate-xy (st from a &optional range0 range1)
  (if (and
        (xy-p st)
        (xy-p from)
        (numberp a))
        ...
        (map 'list
             (lambda (li)
               ...
               (if (and
                 ...
                 (if (= 0 (rem k 2))
                   (+ (nth 0 (xy-ivec from)) 
                      (- (* (- li (nth 0 (xy-ivec from))) (cos a)) 
                         (* (- (nth (1+ k) (xy-ivec st1)) (nth 1 (xy-ivec from))) (sin a))))
                   (+ (nth 1 (xy-ivec from)) 
                      (+ (* (- (nth (1- k) (xy-ivec st1)) (nth 0 (xy-ivec from))) (sin a)) 
                         (* (- li (nth 1 (xy-ivec from))) (cos a)))))
                   li)
                 (if (= 0 (rem k 2))
                   (+ (nth 0 (xy-ivec from)) 
                      (- (* (- li (nth 0 (xy-ivec from))) (cos a)) 
