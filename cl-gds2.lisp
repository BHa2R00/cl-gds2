...
(defstruct boundary 
  ...
  datatype          
  propattr propvalue)
(defstruct timestamp year month day hour minute sec)
(defstruct endel)
(defstruct endstr)
(defstruct strname ascii)
(defstruct str times name elms)
(defstruct units sizes strs)
(defstruct generations copies)
(defstruct endlib)
(defstruct libname ascii)
(defstruct lib times name elms)
(defstruct header version)
(defstruct gds header libs)
(defun rdgdsrec (s end)
  (let*((bsz (rd-ux s 2 end))
    ...
    (case idx
      ...
      (#x1003 (make-xy :ivec (uvec2ivec (rd-uxn s 2 end (ash bsz -1)) 15)))
      (#x0f03 (make-width :ivec (uvec2ivec (rd-uxn s 2 end (ash bsz -1)) 15)))
  ...
  (cond
    ...
    ((xy-p r)
     (let ((sz (length (xy-ivec r))))
       (wt-ux s 2 (+ 4 (ash sz 1)))
       (wt-ux s 2 #x1003)
       (wt-uxn s 2 (ivec2uvec (xy-ivec r) 15) sz)
       ))
    ((width-p r)
     (let ((sz (length (width-ivec r))))
       (wt-ux s 2 (+ 4 (ash sz 1)))
       (wt-ux s 2 #x0f03)
       (wt-uxn s 2 (ivec2uvec (width-ivec r) 15) sz)
       ))
    ((angle-p r)
     (let ((bsz 8))
       (wt-ux s 2 (+ 4 bsz))
  ...
  (let ((r (list)))
    (do ((e (rdgdsrec s 'end)
      ...
      ((equalp e 'end)
       (reverse r))
      (push e r))))
(defun write-gds (s elms)
  (map 'list (lambda (e) (wtgdsrec s e)) elms))
